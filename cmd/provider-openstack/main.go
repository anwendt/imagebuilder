...
import (
	...
	"github.com/anwendt/imagebuilder/plugins/openstack"
...
func main() {
	...
	if metricsListen != "" {
		go func() {
			...
			slog.Info("starting openstack provider metrics", slog.String("address", metricsListen))
	...
	slog.Info("starting openstack provider", slog.String("address", listen))
	if err := sdk.Serve(context.Background(), listener, openstack.NewSDKProvider(), serverOptions...); err != nil {
